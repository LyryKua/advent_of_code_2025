 ...
 * @returns {number[][]}
 */
export function getAllNeighbors(arr, p) {
  let left = addVectors(p, [0, -1]);
  let right = addVectors(p, [0, 1]);
  let bottom = addVectors(p, [1, 0]);
  let top = addVectors(p, [-1, 0]);

  let leftTop = addVectors(p, [-1, -1]);
  let rightTop = addVectors(p, [-1, 1]);
  let leftBottom = addVectors(p, [1, -1]);
  let rightBottom = addVectors(p, [1, 1]);

  return [top, right, bottom, left, leftTop, rightTop, leftBottom, rightBottom];
}
